...
BEGIN
	...
	SET @body = (SELECT * FROM UpdatePromiseQueue WHERE SubmissionResult = 0 FOR XML PATH('Element'), ROOT('PromiseChangeElements'))
	EXEC sp_xml_preparedocument @docHandle OUTPUT, @body; 
	-- SELECT QueueID FROM OPENXML(@docHandle, N'PromiseChangeElements/Element', 2) WITH (QueueID int)
